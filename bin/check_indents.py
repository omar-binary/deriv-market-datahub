...
def check_file(filename: str) -> bool:
    """Checks a single file, returning True and writing a cleaned file if a mix of tabs and spaces was found."""
    problem_found = False
    found_spaces = False
    found_tabs = False
    ...
    try:
        ...
        if problem_found:
            ...
            with open(filename, 'w', encoding='utf-8') as f:
                f.writelines(
                    whitespace.replace('\t', '    ') + rest for whitespace, rest in map(find_whitespace, contents)
                )
